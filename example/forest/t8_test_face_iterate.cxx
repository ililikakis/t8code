...
t8_test_fiterate (t8_forest_t forest)
{
  ...
  for (itree = 0; itree < num_trees; itree++) {
    ...
    last_el = t8_forest_get_element_in_tree (forest, itree,
...
static void
t8_test_fiterate_refine_and_partition (t8_cmesh_t cmesh, int level,
 ...
 * the ghost layer. */
static void
t8_test_fiterate_brick (int dim, int x, int y, int z,
...
static void
t8_test_fiterate_hypercube (t8_eclass_t eclass, int level, sc_MPI_Comm comm,
                            int no_vtk)
{
  t8_cmesh_t          cmesh;
 ...
 * partition the forest, create ghost layer and print it. */
static void
t8_test_fiterate_msh_file (const char *fileprefix, int level, int dim,
...
main (int argc, char **argv)
{
  ...
  if (helpme) {
    ...
    if (x_dim == 0 && !strcmp (prefix, "")) {
      ...
      t8_test_fiterate_hypercube ((t8_eclass_t) eclass_int, level,
      ...
      t8_test_fiterate_brick (dim, x_dim, y_dim, z_dim, x_per, y_per, z_per,
      ...
      t8_global_productionf ("Testing ghost on cmesh read from %s.msh\n",
                             prefix);
      t8_test_fiterate_msh_file (prefix, level, dim, sc_MPI_COMM_WORLD,
                                 no_vtk);
    }
  }
